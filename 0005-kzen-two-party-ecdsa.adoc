...
This creates a new encrypted private key c~1~'.
...
P2 sends c~1~'` to P1 (think of it as his challenge to P1).

P1 decrypts c~1~` to this random private key and figures out the public key.

If P1 can produce the same public key for c~1~' that P2 calculated then c must encrypt P1's public key.
