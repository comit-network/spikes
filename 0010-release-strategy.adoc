...
* Version cnd and btsieve crates
* When releasing cnd, also attached btsieve bin
* When releasing btsieve, releast by itself even if latest cnd is not yet compatible
* Expose btsieve version on its REST API
* cnd to check version of btsieve at start-up and confirm compatibility

Validation before releasing:

* A CI run on master/commit candidate should be done before releasing.
...
* Do some swaps on testnet using comit-i and bobtimus
** The test should be exhaustive enough to cover all existing assets (e.g, ETH->BTC and BTC->ERC20 is enough) for both redeem and refund.
** Ideally, we should rotate (one release we do ETH->BTC, BTC->ERC20 and the next time BTC-ETH, ERC20->BTC)
** Have the release candidate locally and keep previous version on bobtimus (except if not backward compatible)
