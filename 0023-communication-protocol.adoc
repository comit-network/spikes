...
In case the digest matches, Bob generates a UUIDv4 and sends it back to Alice.
If the digest does not match, Bob closes the substream without sending any message.
...
Which identities need to be shared depends on:

* the protocols of the swap
* the alpha and beta ledger
* the role

`/comit/bitcoin/identity/1.0.0`
`/comit/ethereum/identity/1.0.0`
`/comit/lightning/identity/1.0.0`
